...
def test_exchange(rpc, alice, bob, swap, aave_coins, sending, receiving, other, fee, admin_fee):
    if fee or admin_fee:
    ...
    aave_coins[sending]._mint_for_testing(bob, amount, {'from': bob})
    swap.exchange(sending, receiving, amount, 0, {'from': bob})

    assert aave_coins[sending].balanceOf(bob) == 0
    assert aave_coins[other].balanceOf(bob) == 0

    received = aave_coins[receiving].balanceOf(bob)
...
def test_min_dy(bob, swap, aave_coins, sending, receiving):
    amount = 10**PRECISIONS[sending]
    aave_coins[sending]._mint_for_testing(bob, amount, {'from': bob})
    ...
    assert aave_coins[receiving].balanceOf(bob) == min_dy
