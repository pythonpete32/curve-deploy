...
def test_kill_approaching_deadline(rpc, alice, swap):
    rpc.sleep(59 * 86400)
    swap.kill_me({'from': alice})
...
def test_kill_beyond_deadline(rpc, alice, swap):
    rpc.sleep(60 * 86400)
...
def test_remove_liquidity(alice, swap, aave_coins):
    amounts = [10**18, 10**6, 10**6]
    ...
    swap.kill_me({'from': alice})
    swap.remove_liquidity(3 * 10**18, [0, 0, 0], {'from': alice})
...
def test_exchange_underlying(alice, swap):
    swap.kill_me({'from': alice})

    with brownie.reverts("dev: is killed"):
        swap.exchange_underlying(0, 0, 0, 0, {'from': alice})
