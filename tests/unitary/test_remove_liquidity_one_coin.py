...
def test_remove_one_coin(alice, swap, aave_coins, pool_token, idx, divisor):
    ...
    assert pool_token.balanceOf(alice) == 3 * 10**24 - amount
...
@pytest.mark.parametrize("idx", range(len(PRECISIONS)))
def test_amount_exceeds_balance(bob, swap, aave_coins, pool_token, idx):
