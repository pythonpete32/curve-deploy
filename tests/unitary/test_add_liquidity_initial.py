...
def setup(alice, aave_coins, swap):
    for coin in aave_coins:
        coin._mint_for_testing(alice, 10**24, {'from': alice})
        coin.approve(swap, 10**24, {'from': alice})


@pytest.mark.parametrize("min_amount", [0, 3 * 10**18])
def test_add_initial_liquidity(alice, swap, aave_coins, pool_token, min_amount):
    amounts = [10**i for i in PRECISIONS]
    swap.add_liquidity(amounts, min_amount, {'from': alice})

    for coin, amount in zip(aave_coins, amounts):
        assert coin.balanceOf(alice) == 10**24 - amount
        assert coin.balanceOf(swap) == amount

    assert pool_token.balanceOf(alice) == 3 * 10**18
    assert pool_token.totalSupply() == 3 * 10**18


@pytest.mark.parametrize("idx", range(len(PRECISIONS)))
def test_initial_liquidity_missing_coin(alice, swap, aave_coins, pool_token, idx):
    amounts = [10**i for i in PRECISIONS]
    amounts[idx] = 0
