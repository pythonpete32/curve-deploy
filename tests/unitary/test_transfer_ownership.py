...
def test_apply(rpc, alice, bob, swap):
    swap.commit_transfer_ownership(bob, {'from': alice})
    rpc.sleep(COMMIT_WAIT)
...
def test_apply_only_owner(rpc, alice, bob, swap):
    swap.commit_transfer_ownership(bob, {'from': alice})
    rpc.sleep(COMMIT_WAIT)
...
def test_apply_insufficient_time(rpc, alice, bob, swap):
    swap.commit_transfer_ownership(bob, {'from': alice})
    rpc.sleep(COMMIT_WAIT-5)

    with brownie.reverts("dev: insufficient time"):
        swap.apply_transfer_ownership({'from': alice})


def test_apply_no_active_transfer(rpc, alice, swap):
