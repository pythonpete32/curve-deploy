...
def setup(alice, bob, aave_coins, swap):
...
def test_add_liquidity(bob, swap, aave_coins, pool_token):
    ...
    assert pool_token.balanceOf(bob) == 3 * 10**24
    assert pool_token.totalSupply() == 6 * 10**24


def test_add_liquidity_with_slippage(bob, swap, aave_coins, pool_token):
    amounts = [10**i for i in PRECISIONS]
    amounts[1] -= 5 * 10**5
    amounts[2] += 5 * 10**5
    swap.add_liquidity(amounts, 0, {'from': bob})

    assert 0.999 < pool_token.balanceOf(bob) / (3 * 10**18) < 1


@pytest.mark.parametrize("idx", range((len(PRECISIONS))))
def test_add_liquidity_one_coin(bob, swap, aave_coins, pool_token, idx):
    amounts = [0, 0, 0]
...
def test_insufficient_balance(charlie, swap, aave_coins, pool_token):
    amounts = [(10**i) for i in PRECISIONS]
...
def test_min_amount_too_high(alice, swap, aave_coins, pool_token, min_amount):
    amounts = [10**i for i in PRECISIONS]
    with brownie.reverts("Slippage screwed you"):
        swap.add_liquidity(amounts, min_amount, {'from': alice})


def test_min_amount_with_slippage(bob, swap, aave_coins, pool_token):
    amounts = [10**i for i in PRECISIONS]
    amounts[1] -= 5 * 10**5
    amounts[2] += 5 * 10**5
    with brownie.reverts("Slippage screwed you"):
